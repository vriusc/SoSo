...
public class CardUtil {
     ...
     *卡号消费记录
     */
    Map<String, List<ConsumerInfo>> consumerInfos = new HashMap<String, List<ConsumerInfo>>();
    /**
     *使用场景列表
     */
    ...
    MobileCard card1 = new MobileCard("13911111111", "qyr1", "1", new TalkPackage(), 58.0, 42);
    MobileCard card2 = new MobileCard("13922222222", "qyr2", "2", new NetPackage(), 68.0, 32.0);
    MobileCard card3 = new MobileCard("13933333333", "qyr3", "3", new SuperPackage(), 78.0, 22.0);
    MobileCard card4 = new MobileCard("13944444444", "qyr4", "4", new TalkPackage(), 78.0, 2.0);
    MobileCard card5 = new MobileCard("5", "qyr4", "5", new TalkPackage(), 78.0, 2.0);
    ...
    /**
     * 创建卡号（以139开头 11位）
     * isExist记录现有用户中是否存在此卡号用户 是：true 否：false
     * @return 生成的随机手机卡号
     */
    ...
    public void showAmountDetail(String searchNumber) {
        ...
        meg.append("合计：" + Common.dataFormat(card.getConsumerAmount()) + "元\n");
        ...
        if (isExistCard(number)) {
            ...
            if (!(card.getSerPackage().getClass().getName().equals(pack.getClass().getName()))) {
                ...
                if (card.getMoney() >= pack.getPrice()) {
                    ...
                    // 当月消费金额设置为新套餐月资费
                    card.setConsumerAmount(pack.getPrice());
     ...
     * 添加一条指定卡的消费记录
     */
    public void addConsumerInfo(String number, ConsumerInfo info) {
        Set<String> numbers = consumerInfos.keySet();
        Iterator<String> it = numbers.iterator();
        List<ConsumerInfo> infos = new ArrayList<ConsumerInfo>();
        boolean isExist = false; // 现有消费列表中是否存在此卡号消费记录，是：true 否：false
        while (it.hasNext()) {
            if (it.next().equals(number)) {
                // 消费集合中已有该卡号消费记录，则找到该卡号的消费记录集合，添加一条即可
                infos = consumerInfos.get(number);
                infos.add(info);
                isExist = true;
        ...
        if (!isExist) {
            infos.add(info);
            consumerInfos.put(number, infos);
            System.out.println("不存在此卡的消费记录，已添加一条消费记录。");
        }
    }


    public void printConsumerInfo(String number){
        Writer fileWriter = null;
        try {
            fileWriter = new FileWriter(number+"消费记录.txt");
            Set<String> numbers = consumerInfos.keySet();
            Iterator<String> it = numbers.iterator();
            List<ConsumerInfo> infos = new ArrayList<ConsumerInfo>();//存储指定卡所有消费记录
            boolean isExist = false; // 现有消费列表中是否存在此卡号消费记录，是：true 否：false
            while (it.hasNext()) {
                if (it.next().equals(number)) {
                    infos = consumerInfos.get(number);
            ...
            switch (ranNum) {
                ...
                case 1:
                    ...
                    if (pack instanceof CallService) {
                        ...
                        // 添加一条消费记录
                        addConsumerInfo(number, new ConsumerInfo(number,
                                scene.getType(), temp));
                        break;
                    } else {
                ...
                case 3:
                    ...
                    if (pack instanceof SendService) {
                        ...
                        // 添加一条消费记录
                        addConsumerInfo(number, new ConsumerInfo(number,scene.getType(), temp));
                        break;
                    } else {
                ...
                case 5:
                    ...
                    if (pack instanceof NetService) {
                        ...
                        // 执行上网方法
                        try {
                            temp = netService.net(scene.getData(), card);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        // 添加一条消费记录
    ...
    public void showDescription(){
        Reader rd = null;
        try {
            rd = new FileReader("src/main/resources/Description.txt");
